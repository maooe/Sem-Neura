...
import { LayoutDashboard, Calendar, CreditCard, TrendingUp, Settings, BrainCircuit, ChevronDown, Palette, Check, LogIn, LogOut, User as UserIcon, Thermometer } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const menuItems = [
    ...
    { id: 'analysis', label: 'Performance', icon: Thermometer },
    { id: 'annual', label: 'Mapa Anual 2026', icon: Calendar },
  ...
  return (
    <aside className="w-64 bg-slate-900 text-slate-300 flex-shrink-0 hidden lg:flex flex-col h-screen sticky top-0 overflow-y-auto custom-scrollbar border-r border-white/5">
      ...
      <div className="px-6 mb-8 space-y-4">
        {!currentUser ? (
          <button 
            ...
            className="w-full flex items-center gap-3 p-3 bg-brand-600 hover:bg-brand-500 text-white rounded-2xl transition-all shadow-brand font-black text-[10px] uppercase tracking-widest"
          >
            <LogIn size={18} />
            Login Cloud
          </button>
          ...
          <div className="p-3 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-between group">
             <div className="flex items-center gap-3 overflow-hidden">
                ...
                <div className="text-left overflow-hidden">
                  <p className="text-[10px] font-black text-slate-500 uppercase leading-none mb-1">Logado</p>
      ...
      <nav className="flex-1 px-4 space-y-2">
        <p className="px-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4">Módulos</p>
        {menuItems.map((item) => (
          <button
            key={item.id}
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-black text-xs uppercase tracking-widest ${
              activeView === item.id 
                ? 'bg-brand-600 text-white shadow-brand shadow-lg' 
        ...
        <div className="pt-8 px-4">
           <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4">Temas</p>
      ...
      <div className="p-4 mt-auto">
        <div className={`p-4 rounded-2xl border transition-colors ${isSyncActive ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-slate-800/50 border-slate-700/50'}`}>
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[10px] font-black uppercase tracking-widest ${isSyncActive ? 'text-emerald-400' : 'text-slate-500'}`}>
              {isSyncActive ? 'Cloud Online' : 'Local Only'}
            </span>
          </div>
        </div>
      </div>
